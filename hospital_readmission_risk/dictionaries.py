...
def load_config(dict_type: str = "procedures", source_type: str = "train"):
    """
    Load state path, targets, and IO config for a dictionary type.

    Returns:
        (STATE_PATH, targets, data_path, sql, write_path)
    """
...
def load_main_config(dict_type: str = "main_diagnoses", source_type: str = "train"):
    """
    Load config for building advanced diagnosis tables.
...
def build_related_diagnoses(data: pd.DataFrame, state_path: Path) -> pd.DataFrame:
    """
