...
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis1 IS NOT NULL AND currentillnessdate <= window_end
UNION ALL
SELECT encounter AS stay_id, CAST(diagnosis2 AS INT64) AS code
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis2 IS NOT NULL AND currentillnessdate <= window_end
UNION ALL
SELECT encounter AS stay_id, CAST(diagnosis3 AS INT64) AS code
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis3 IS NOT NULL AND currentillnessdate <= window_end
UNION ALL
SELECT encounter AS stay_id, CAST(diagnosis4 AS INT64) AS code
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis4 IS NOT NULL AND currentillnessdate <= window_end
UNION ALL
SELECT encounter AS stay_id, CAST(diagnosis5 AS INT64) AS code
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis5 IS NOT NULL AND currentillnessdate <= window_end
UNION ALL
SELECT encounter AS stay_id, CAST(diagnosis6 AS INT64) AS code
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis6 IS NOT NULL AND currentillnessdate <= window_end
UNION ALL
SELECT encounter AS stay_id, CAST(diagnosis7 AS INT64) AS code
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis7 IS NOT NULL AND currentillnessdate <= window_end
UNION ALL
SELECT encounter AS stay_id, CAST(diagnosis8 AS INT64) AS code
FROM {{DATASET_SLIM}}.claims_slim WHERE diagnosis8 IS NOT NULL AND currentillnessdate <= window_end
      ) t
  ),
  ...
  procedures AS (
    ...
    HAVING DATE(MAX(proc.stop)) <= window_end
  ),
  ...
  -- For each encounter in the window, count how many distinct chronic conditions the patient had before or at that encounter
  ...
  patient_condition_starts AS (
    ...
    WHERE claims.currentillnessdate <= window_end
    GROUP BY claims.patientid
  ),
  -- Flag each encounter in the window: 1 if the patient already had the specific disease before this encounter started
  ...
  -- Find encounters that follow a planning procedure (is_planning = 1); full history up to window_end
  ...
  -- For each encounter in the window, compute days since each prior surgery (is_surgery = 1 in procedures_dictionary)
...
-- Final assembly: join all CTEs onto encounters_slim; one output row per encounter in the window
SELECT
  e.id AS stay_id,
