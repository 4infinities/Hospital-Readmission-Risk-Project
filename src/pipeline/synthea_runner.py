...
class SyntheaRunner:
        ...
        if self.files_to_copy is None:
            ...
            for name in self.files_to_copy:
                ...
                if src.is_file():
                    ...
                    self.logger.warning("Requested file_to_copy not found: %s", src)

        # Copy every CSV file (patients.csv, encounters.csv, etc.)
        for csv_file in files:
            dest = target_dir / csv_file.name
            shutil.copy2(csv_file, dest)
            self.logger.info("Copied %s -> %s", csv_file, dest)

            if self.delete_source_files:
                try:
                    csv_file.unlink()
                    self.logger.info("Deleted source file: %s", csv_file)
                except OSError as e:
                    self.logger.warning(
    ...
    def run(
        ...
        self.logger.info("Running Synthea with command: %s", " ".join(cmd))
        self.logger.info("Synthea working directory: %s", self.synthea_home)
        self.logger.info("Synthea CSV directory: %s", self.synthea_csv_dir)
        self.logger.info("Profile output_dir: %s", self.output_dir)
        self.logger.info("Files to copy: %s", self.files_to_copy or "ALL *.csv")
        self.logger.info("Delete source files after copy: %s", self.delete_source_files)

        if dry_run:
            # Do not execute anything, just show where we would copy
            target_dir = self.output_dir / state
            self.logger.info("[DRY RUN] Would copy CSVs into: %s", target_dir)
            return target_dir
