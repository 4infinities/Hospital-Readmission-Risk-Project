CREATE OR REPLACE TABLE hospital-readmission-4.helper_tables.helper_clinical
AS (
  WITH
    end_date AS (
    ...
    procedures AS (
      ...
      FROM `hospital-readmission-4.data_slim.procedures_slim` proc
      LEFT JOIN
        `hospital-readmission-4.data_slim.procedures_dictionary` proc_dict
    ...
    chronic_patient_codes AS (
      ...
      JOIN `hospital-readmission-4.data_slim.diagnoses_dictionary` dict
    ...
    patient_condition_starts AS (
      ...
      JOIN `hospital-readmission-4.data_slim.claims_slim` claims
    ...
    plans_by_procedure AS (
      ...
      FROM `hospital-readmission-4.data_slim.encounters_slim` e
      LEFT JOIN `hospital-readmission-4.data_slim.procedures_slim` proc
    ...
    plans_by_careplans AS (
      SELECT
        e.id AS stay_id,
        max(
          CASE
            WHEN
              care.start < date(e.start)
              AND coalesce(
                care.stop, timestamp_add(care.start, INTERVAL 60 day))
                > date(e.start)
              THEN 1
            ELSE 0
            END) AS is_planned,
      FROM `hospital-readmission-4.data_slim.encounters_slim` e
      LEFT JOIN `hospital-readmission-4.data_slim.careplans_slim` care
    ...
    planned_stays AS (
      SELECT
        care.stay_id,
        greatest(coalesce(proc.is_planned, 0), coalesce(care.is_planned, 0))
          AS is_planned
      FROM plans_by_careplans care
      LEFT JOIN plans_by_procedure_flag AS proc
        ON care.stay_id = proc.stay_id
    ),
  ...
  LEFT JOIN `hospital-readmission-4.data_slim.main_diagnoses` main
    ON e.id = main.stay_id
  LEFT JOIN `hospital-readmission-4.data_slim.diagnoses_dictionary` dict
